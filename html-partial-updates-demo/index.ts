...
async function fetchNode(nodeTag: string): Promise<ApiResponse<Document>> {
	const res: Response = await fetch(`${localhost}:${port}${pathname}?node=${nodeTag}`, {
		...
		headers: {
			Connection:
				parseInt(nodeTag.substring(4)) <= getTagLimit(tagLimit)
					? "keep-alive"
					: "close",
		},
	});
...
if (args.length > 1) {
	console.error(
		"Please provide only one node tag as an argument or 'all' as argument.",
	);
	process.exit(1);
}
if (args[0] === "all") {
	const nodeTags: Array<string> = Array.from(
		{ length: getTagLimit(tagLimit) },
		(_, i) => `node${i + 1}`,
	);
	...
	Promise.allSettled(urls)
		.then((responses: Array<PromiseSettledResult<ApiResponse<Document>>>) => {
			return responses.reduce(
				(
					acc: Array<Document>,
					current: PromiseSettledResult<ApiResponse<Document>>,
				) => {
